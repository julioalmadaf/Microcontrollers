...
I2C_t i2c = {0x0B, 0x90, 0}; //variable tipo I2C_t estructura con variables inicializadas
...
void main(void) {
	...
	TEMP_LIMIT=5000; //Temperatura limite inicial
	velocidad=6; //velocidad inicial
	
	init_IIC_STRUCT(i2c); //inicializacion de IIC
	
	IIC_send_byte(0x7E); //direccion del modulo
	
	LCD_Init(); //inicializacion de IIC
	...
	for(;;) {	
		...
		if(FRPS==1)
		{
			velocidad=2000/valor;// CONVERSION
			FRPS=0;
		}
		...
		{	
			...
			CounterTemp++;
			samples=1;
			if(CounterTemp>=5) //***checar si puedo imprimir antes la tempe
			{ 
				temperature[6]= '\r';
				temperature[5]= '\n';
